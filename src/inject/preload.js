...
const { ipcRenderer, webFrame, clipboard, nativeImage } = require('electron');
          ...
          if (!$(".dropdown_menu>.uoscopyitem").length) {
            ...
            $('.uoscopyitem').click(function () {
              ...
              if (window.getSelection().type === 'Range') {
                content = copyitem.innerText.substr(Math.min(window.getSelection().baseOffset, window.getSelection().focusOffset), Math.abs(window.getSelection().baseOffset - window.getSelection().focusOffset))
              } else {
              ...
              if (src) {
                ...
                // ipcRenderer.send('copy-image', { x: $(e.target).offset().left, y: $(e.target).offset().top });

                // clipboard.writeImage(nativeImage.createFromPath('https://tm-image.tianyancha.com/tm/f54e0db9b1315965ae64243a2fc73824.jpg'))
              } else {
