...
class Injector {
				...
				angular.module(moduleName).config(['$httpProvider', ($httpProvider) => {
					...
					if (AppConfig.readSettings('history') === 'on') {
						$rootScope.$on("message:add:success", function(e, originMsg) {
							for(let key in self.ChatHistorys.AllChatHistorys){
								for(let i=0,len=self.ChatHistorys.AllChatHistorys[key].chats.length;i<len;i++){
									if(self.ChatHistorys.AllChatHistorys[key].chats[i].MsgId == originMsg.MsgId){
										self.ChatHistorys.AllChatHistorys[key].chats.splice(i,1)
										return
									}
								}
							}
							self.ChatHistorys.saveHistory({
								type: 'add',
								originMsg
							});
						});
	...
	initInjectBundle() {
		const initModules = () => {
			if (!window.$) {
				return setTimeout(initModules, 10000);
			}
		...
		window.onload = () => {
			...
			$('body').contextmenu(function(e) {
				let classlist1 = $(e.target).parents('.bubble_cont')[0].classList
				let classlist2 = e.target.classList
				if (classlist1.contains('bubble_cont') || classlist2.contains('bubble_cont')) {
					if (!$(".dropdown_menu>.uoscopyitem").length) {
						$(".dropdown_menu").append(`<li ng-repeat="item in contextMenuList" class="uoscopyitem">
                  <a class="ng-binding ng-scope">${Common.MENU.copy}</a>
              </li>`)
						$('.uoscopyitem').click(function() {
							let content = ''
							let copyitem = $(e.target)[0]
							if (window.getSelection().type === 'Range') {
								content = copyitem.innerText.substr(Math.min(window
									.getSelection().baseOffset, window.getSelection()
									.focusOffset), Math.abs(window.getSelection()
									.baseOffset - window.getSelection().focusOffset))
							} else {
								content = copyitem.innerText
							}
				...
				case constants.MSGTYPE_EMOTICON:
					if (/&lt;msg&gt;&lt;emoji fromusername/.test(msg.Content)) {
						...
						if (msg.ImgHeight >= Common.EMOJI_MAXIUM_SIZE) {
							Injector.lock(msg, 'MMImgStyle', {
								height: `${Common.EMOJI_MAXIUM_SIZE}px`,
								width: 'auto'
							});
						} else if (msg.ImgWidth >= Common.EMOJI_MAXIUM_SIZE) {
							Injector.lock(msg, 'MMImgStyle', {
								height: 'auto',
								width: `${Common.EMOJI_MAXIUM_SIZE}px`
							});
						} else {
							Injector.lock(msg, 'MMImgStyle', {
								height: msg.ImgHeight,
								width: 'auto'
							});
						}
					}
				...
				case constants.MSGTYPE_RECALLED:
					if (AppConfig.readSettings('prevent-recall') === 'on') {
						try {
							...
							Injector.lock(msg, 'MsgType', constants.MSGTYPE_SYS);
							Injector.lock(msg, 'MMActualContent', msg.Content.match(/\!\[CDATA\[(.*)?]\]/)[
								1]);
	...
	initSeteditArea() { //初始化缩放输入文本域事件
		...
		angular.element('#chatArea>.box_ft')[0].style.height = 180 + parseInt(AppConfig.readSettings(
			'chat-area-offset-y')) + 'px' //赋初值方便计算
		angular.element('#chatArea>.chat_bd')[0].style.marginBottom = parseInt(AppConfig.readSettings(
			'chat-area-offset-y')) + 'px'
		...
		$chatArea.addEventListener('mousemove', (event) => {
			if (mousedown) {
				...
				angular.element('#chatArea>.chat_bd')[0].style.marginBottom = startChat_bd + offsetY +
					'px' //聊天区
		...
		$chatArea.addEventListener('mouseup', (event) => {
			if (mousedown) {
				mousedown = false
				AppConfig.saveSettings('chat-area-offset-y', parseInt(angular.element('#chatArea>.chat_bd')[
	...
	initcopy() {
		...
		//       console.log(angular.element('.dropdown_menu>li:eq(' + i + ')'))
		//       console.log(angular.element('.dropdown_menu>li:eq(' + i + ')').scope())
	...
	initIPC() {
		...
		ipcRenderer.on('show-wechat-window', () => {
			if (this.lastUser && this.lastUser !== '2233' && angular.element('#chatArea').scope()
				.currentUser === '2233') {
				angular.element('.chat_list').scope().itemClick(this.lastUser);
				this.lastUser = ''
			}
